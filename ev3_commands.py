...
#             ev3.opOutput_Clr_Count
    ...
    def get_wheel_position(self, port:int) -> int:
    ...
    def get_gyro_rate(self, port:int):
        ops = b''.join([
            ev3.opInput_Device,
            ev3.READY_RAW,
            ev3.LCX(0),                # LAYER
            ev3.LCX(port),       # NO ( port 2)
    ...
    def get_gyro_angle(self, port:int):
         ops = b''.join([
             ev3.opInput_Device,
             ev3.READY_RAW,
             ev3.LCX(0),                # LAYER
...
#          print(reply)
    ...
    def get_gyro_angle_rate(self, port:int):
        ops = b''.join([
            ev3.opInput_Device,
            ev3.READY_RAW,
            ev3.LCX(0),                # LAYER
    ...
    def reset_gyro_angle(self, port:int):
        # switch modes to reset gyro angle
        self.get_gyro_angle_rate(port)
        self.get_gyro_rate(port)

    def get_color_data(self, port:int):
        ops = b''.join([
             ev3.opInput_Device,
             ev3.READY_SI,
             ev3.LCX(0),                # LAYER
             ev3.LCX(port),       # NO ( port 3)
